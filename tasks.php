...
(function () {
  ...
  function wireAuditHistoryFilters(scope) {
    ...
    root.querySelectorAll('.task-audit-history-shell').forEach(function (shell) {
      ...
      const shortcutHintCompactText = 'Shortcuts: A S R C G H M Y Shift+Y Shift+M ? Esc';
      const shortcutHintDetailedText = 'Shortcuts: A = All Events, S = Status Changes, R = Reset view, C = Clear overrides, G = Global mode, H = Hint detail, M = Toast mute, Y = Copy source, Shift+Y = Copy snapshot, Shift+H = Compact, Shift+M = Unmute';
      ...
      function sourceTooltipText(sourceLabel) {
        if (sourceLabel === 'Keyboard') {
          return 'Keyboard: changed with panel shortcuts (A/S/R/C/G/H/M/Y/Shift+Y/Shift+M/?/Esc).';
        }
        if (sourceLabel === 'Button') {
