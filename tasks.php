...
(function () {
  ...
  function wireAuditHistoryFilters(scope) {
    ...
    root.querySelectorAll('.task-audit-history-shell').forEach(function (shell) {
      ...
      const shortcutHintCompactText = 'Shortcuts: A S R C G H M Y O Shift+Y Shift+O Ctrl+Y Shift+M Shift+P ? Esc';
      const shortcutHintDetailedText = 'Shortcuts: A = All Events, S = Status Changes, R = Reset view, C = Clear overrides, G = Global mode, H = Hint detail, M = Toast mute, Y = Copy source, O = Copy origin, Shift+Y = Copy snapshot, Shift+O = Copy origin context, Ctrl+Y = Copy source+filter, Shift+H = Compact, Shift+M = Unmute, Shift+P = Reload policy';
      ...
      function sourceTooltipText(sourceLabel) {
        if (sourceLabel === 'Keyboard') {
          return 'Keyboard: changed with panel shortcuts (A/S/R/C/G/H/M/Y/O/Shift+Y/Shift+O/Ctrl+Y/Shift+M/Shift+P/?/Esc).';
        }
        if (sourceLabel === 'Button') {
