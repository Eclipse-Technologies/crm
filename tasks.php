...
function renderTaskAuditHistoryHtml(array $entries): string {
  ...
  return '<div class="task-audit-history-shell" tabindex="0" aria-label="Task audit history panel" data-origin-toast-cooldown-ms="550" data-origin-success-live-cooldown-ms="650" data-origin-failure-live-cooldown-ms="700" data-origin-recovery-window-ms="2400">'
    ...
    . '</div>'
    . '<div class="js-audit-cheatline" style="display:none;font-size:10px;color:#334155;background:#f1f5f9;border:1px solid #e2e8f0;border-radius:999px;padding:2px 8px;margin:0 0 6px 0;width:max-content;">Keys: A S R C G H M Y O Shift+Y Shift+O Ctrl+Y Shift+M Shift+P Shift+K Shift+J ? Esc</div>'
    . '<div class="js-audit-shortcut-hint" style="display:none;font-size:10px;color:#64748b;margin:0 0 6px 0;">Shortcuts: A S R C G H M Y O Shift+Y Shift+O Ctrl+Y Shift+M Shift+P Shift+K Shift+J ? Esc <span class="js-audit-hint-mode-badge" style="display:inline-flex;align-items:center;margin-left:6px;padding:1px 6px;border-radius:999px;border:1px solid #cbd5e1;background:#f8fafc;color:#334155;font-size:10px;font-weight:600;">Hint: Compact</span> <button type="button" class="js-audit-hint-reset" style="margin-left:6px;border:none;background:transparent;color:#0f766e;font-size:10px;font-weight:700;padding:0;cursor:pointer;">Reset hint</button> <button type="button" class="js-audit-hint-toast-toggle" aria-pressed="false" style="margin-left:6px;border:none;background:transparent;color:#64748b;font-size:10px;font-weight:700;padding:0;cursor:pointer;">Mute hint toasts</button> <span class="js-audit-hint-toast-muted-chip" style="display:none;align-items:center;margin-left:6px;padding:1px 6px;border-radius:999px;border:1px solid #7c2d12;background:#fef2f2;color:#7c2d12;font-size:10px;font-weight:700;">Muted</span></div>'
    . '<div class="js-audit-shortcut-help" style="display:none;font-size:10px;color:#334155;background:#f8fafc;border:1px solid #e2e8f0;border-radius:6px;padding:6px 8px;margin:0 0 6px 0;">Shortcut help: A = All Events, S = Status Changes, R = Reset view, C = Clear overrides, G = Toggle global mode, H = Toggle hint detail, M = Toggle hint toasts mute, Y = Copy current source label, O = Copy current origin label, Shift+Y = Copy source/hint/toasts snapshot, Shift+O = Copy origin/source/filter context, Ctrl+Y = Copy source/filter/origin context, Shift+H = Reset hint compact, Shift+M = Unmute hint toasts, Shift+P = Reload copy policy from shell attributes, Shift+K = Copy policy readout text, Shift+J = Toggle manual hint (escalated mode), Show manual hint button = Restore hidden manual hint (Enter/Space supported), Reset hint button = Compact, ? = Toggle this help. Ctx note: Origin Context = Origin + Source + Filter.<div class="js-audit-policy-readout" style="margin-top:4px;padding-top:4px;border-top:1px dashed #cbd5e1;color:#334155;font-weight:600;">Policy: Toast 550ms | Success 650ms | Failure 700ms | Recovery 2400ms</div><div class="js-audit-policy-manual-hint" style="display:none;margin-top:4px;font-size:10px;color:#7f1d1d;">Copy may be blocked here. Select this policy line, then press Ctrl+C. <button type="button" class="js-audit-policy-manual-dismiss" style="margin-left:6px;border:none;background:transparent;color:#991b1b;font-size:10px;font-weight:700;padding:0;cursor:pointer;">Dismiss</button></div><button type="button" class="js-audit-policy-manual-restore" style="display:none;margin-top:4px;border:none;background:transparent;color:#0f766e;font-size:10px;font-weight:700;padding:0;cursor:pointer;">Show manual hint (Shift+J)</button></div>'
...
(function () {
  ...
  function wireAuditHistoryFilters(scope) {
    ...
    root.querySelectorAll('.task-audit-history-shell').forEach(function (shell) {
      ...
      const shortcutPolicyManualHint = shell.querySelector('.js-audit-policy-manual-hint');
      const shortcutPolicyManualDismiss = shell.querySelector('.js-audit-policy-manual-dismiss');
      const shortcutPolicyManualRestore = shell.querySelector('.js-audit-policy-manual-restore');
      ...
      const shortcutHintCompactText = 'Shortcuts: A S R C G H M Y O Shift+Y Shift+O Ctrl+Y Shift+M Shift+P Shift+K Shift+J ? Esc';
      const shortcutHintDetailedText = 'Shortcuts: A = All Events, S = Status Changes, R = Reset view, C = Clear overrides, G = Global mode, H = Hint detail, M = Toast mute, Y = Copy source, O = Copy origin, Shift+Y = Copy snapshot, Shift+O = Copy origin context, Ctrl+Y = Copy source+filter, Shift+H = Compact, Shift+M = Unmute, Shift+P = Reload policy, Shift+K = Copy policy, Shift+J = Toggle manual hint';
      ...
      let policyManualHintDismissed = false;
      ...
      function refreshPolicyReadoutTone() {
        ...
        if (shortcutPolicyManualRestore) {
          const showManualRestore = unavailableHint && policyManualHintDismissed;
          shortcutPolicyManualRestore.style.display = showManualRestore ? 'inline-flex' : 'none';
        }
      ...
      function sourceTooltipText(sourceLabel) {
        if (sourceLabel === 'Keyboard') {
          return 'Keyboard: changed with panel shortcuts (A/S/R/C/G/H/M/Y/O/Shift+Y/Shift+O/Ctrl+Y/Shift+M/Shift+P/Shift+K/Shift+J/?/Esc).';
        }
        if (sourceLabel === 'Button') {
      ...
      function announcePolicyManualHintDismissed(triggerLabel) {
        if (!hintLiveRegion) {
          return;
        }
        const message = 'Policy manual copy hint dismissed. Trigger: ' + String(triggerLabel || 'manual dismiss') + '.';
        if (hintLiveTimer) {
          window.clearTimeout(hintLiveTimer);
        }
        hintLiveTimer = window.setTimeout(function () {
      ...
      if (shortcutPolicyManualDismiss) {
        shortcutPolicyManualDismiss.addEventListener('click', function (event) {
          event.preventDefault();
      ...
      function toggleManualPolicyHint(statusPrefix, triggerLabel) {
        const isEscalated = lastPolicyCopyFailed && policyCopyFailureStreak >= 2;
        if (!isEscalated) {
          setKeyStatus(statusPrefix + ' unavailable');
          showToast('Manual hint toggle is only available in escalated policy-copy mode.', false);
          return;
        }
        policyManualHintDismissed = !policyManualHintDismissed;
        refreshPolicyReadoutTone();
        if (policyManualHintDismissed) {
          ...
          showToast('Manual policy-copy hint shown.', false);
          announcePolicyManualHintVisible(triggerLabel || 'keyboard Shift+J');
        }
      }
      ...
      shell.addEventListener('keydown', function (event) {
        ...
        if (key === 'a') {
          ...
          event.preventDefault();
          toggleManualPolicyHint('Shift+J -> Manual hint', 'keyboard Shift+J');
        } else if (key === 'm') {
          event.preventDefault();
